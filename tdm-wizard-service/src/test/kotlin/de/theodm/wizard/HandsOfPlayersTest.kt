...
internal class HandsOfPlayersTest {
    ...
    fun letzteKarte() {
        val handsOfPlayers = HandsOfPlayers(mapOf(
            wpSebastian to listOf(Joker, Joker),
            wpTabea to listOf(ccBlau12),
            wpDomi to listOf(ccBlau11)
        ))
    ...
    fun zaubererAbgelegt() {
        ...
        handsOfPlayers = handsOfPlayers.playCardFromHand(wpSebastian, Joker)

        Truth.assertThat(handsOfPlayers[wpSebastian]).containsExactly(Joker)
    }
}